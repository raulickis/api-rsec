...
type UsuarioForm struct {
	...
	Documento      string           `json:"documento"                  validate:"required,max=20,isvalidcpf"`
	Email          string           `json:"email"                      validate:"required,max=100,email"`
	Ddd            string           `json:"ddd"                        validate:"max=2"`
	Telefone       string           `json:"telefone"                   validate:"max=10"`
	FotoUrl        string           `json:"foto_url"                   validate:"required,max=200"`
	Enderecos      []EnderecoForm   `json:"enderecos"                   `
}
...
func (c *UsuarioForm) ValidarUsuario() []string {

	v, trans := utils.ValidatorForm()

	_ = v.RegisterValidation("isvalidcpf", func(fl validator.FieldLevel) bool {return fl.Field().String()=="" || utils.IsValidCPF(fl.Field().String())})
